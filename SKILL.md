...
## 模糊请求 Double Check（必做）

当客服说「帮我抓取」「帮我找客户」但**没说具体关键词/筛选条件**时，**必须先 double check，不要直接跑**。

### 检查历史记录

先查 `data/` 目录下最近的采集文件（`ls -t data/leads_*.csv | head -3`），获取上次的关键词和日期。

### 两个分支

**分支 A：之前爬过（有历史记录）**
> 「上次你抓的是 **{关键词}**，{日期} 抓的。这次还用同样的关键词和筛选条件吗？还是要换一个？」

- 如果客服说「还用之前的」→ 直接用上次参数跑
- 如果客服说了新关键词 → 用新关键词跑

**分支 B：没爬过 / 很久没爬了（无历史或超过 7 天）**
> 「好的，帮你抓外贸通买家线索。需要确认几个参数：
> 1. **产品关键词**：你要找什么产品的买家？（比如 cargo、led light、furniture）
> 2. **采购地区**：买家在哪个国家？（比如 美国、欧洲，不限就跳过）
> 3. **供应地区**：卖家从哪发货？（一般选中国）
> 4. **页数**：抓几页？每页 20 条，默认 5 页 = 100 条」

- 客服给出关键词后 → 确认一遍再跑
- 客服说「随便」「都可以」→ 用默认 `cargo` 跑，但告知「我用 cargo 先试 2 页，你看看结果对不对」

### 原则

- **永远不要在不确认关键词的情况下直接跑**——跑错了浪费配额和时间
- double check 只需一轮对话，不要来回追问超过 2 次
- 如果客服已经给了具体关键词（比如「帮我抓 led light 的美国买家」），直接跑，不需要 double check
